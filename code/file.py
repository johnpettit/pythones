...
for acc in acs:
    ...
    print("Create Date :  " + acc.CreateDate)
    es.index(index='names', doc_type='address', body={'FirstName': acc.FirstName, 'LastName': acc.LastName, 'NickName': acc.NickName, 'Street1': acc.Street1, 'Street2': acc.Street2, 'City': acc.City, 'State':acc.State,'Postal':acc.Postal,'Country':acc.Country,'CreateDate':acc.CreateDate,'ModifiedDate':acc.ModifiedDate})
